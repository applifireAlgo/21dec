Ext.define('Dec22.view.login.LoginPageController', {
						...
						if (jsonResponse.response.changePassword == true) {
							Ext.Msg.confirm('Confirm', jsonResponse.response.message, function(id, value) {
								if (id == 'yes') {
									...
									scope.container.removeAll();
									scope.container.add(Ext.create('Dec22.view.login.ChangePasswordScreen'));
								}
							}, {
								controller : this,
								scope : scope
		...
		Ext.Ajax.request({
			...
			success : function(response, sender) {
				...
				if (responseText.response.success) {
					...
					var forgetPasswordWindow = Ext.create('Dec22.view.login.ForgetPasswordWindow', {
